...
from .forms import LoginForm, ProductForm, EditProductForm
...
from customer.models import CustomerCheckout, customerPayedProducts
...
def addproduct(request):
    if request.method == 'POST':
        ...
        if product_form.is_valid():
            ...
            product_instance.product_picture = request.FILES['product_image']
...
@user_passes_test(checksuperuser, login_url=reverse_lazy('login'))
def adminviewproductdetails(request, product_id):
    product = Products.objects.get(id=product_id)
    return render(request, 'adminpannel/viewproduct.html', {'product': product})
